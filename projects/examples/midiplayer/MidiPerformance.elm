    ...
    in
        if isSeq then
            List.map f notes
                ...
                |> List.reverse
            -- we need to reverse now we do a left fold
        else
...
translateNotePair n1 s1 n2 s2 =
    let
        ...
        ticks1 =
            (noteTicks n1.duration * numerator s1.tempoModifier)
                // (denominator s1.tempoModifier)

        ticks2 =
            (noteTicks n2.duration * numerator s2.tempoModifier)
        ...
        barAccidentals =
            s1.thisBar.accidentals

        note1 =
            MNote { ticks = ticks1, pitch = toMidiPitch n1 s1.modifiedKeySignature barAccidentals, pc = Just n1.pitchClass, accidental = n1.accidental } False

        note2 =
            MNote { ticks = ticks2, pitch = toMidiPitch n2 s2.modifiedKeySignature barAccidentals, pc = Just n2.pitchClass, accidental = n2.accidental } False
    in
        ...
        case m of
            ...
            Tuplet signature tnotes ->
                let
                    ...
                    tupletNotes =
                        translateNoteSequence True tupletState tnotes Nothing

                    newState =
                        addNotesToState tupletNotes state
                in
                    ( midiMelody, newState )
                ...
                case b of
                    LeftArrow i ->
                        let
                            ...
                            leftState =
                                { state | tempoModifier = down }

                            rightState =
                                { state | tempoModifier = up }

                            notePair =
                                translateNotePair n1 leftState n2 rightState

                            newState =
                                addNotesToState notePair state
                        in
                            ( midiMelody, newState )
            ...
            Chord abcChord ->
                let
                    chord =
                        translateNoteSequence False state abcChord.notes (Just abcChord.duration)
